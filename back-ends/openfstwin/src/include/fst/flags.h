...
class FlagRegister {
 ...
 private:
  ...
  OPENFSTDLL static fst::FstOnceType register_init_;   // ensures only called once //ChangedPD
  OPENFSTDLL static fst::Mutex* register_lock_;        // multithreading lock //ChangedPD
  OPENFSTDLL static FlagRegister<T> *register_; //ChangedPD

  std::map< string, FlagDescription<T> > flag_table_;
};

#ifdef OPENFSTEXPORT //AddedPD
template <class T>
...
void OPENFSTDLL ShowUsage(); //ChangedPD
