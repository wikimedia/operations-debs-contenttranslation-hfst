...
class SymbolTableImpl {
 public:
  ...
  int64 OPENFSTDLL AddSymbol(const string& symbol, int64 key); //ChangedPD
  ...
  static OPENFSTDLL SymbolTableImpl* ReadText(istream &strm, //ChangedPD
                                   const string &name,
                                   bool allow_negative = false);

  static OPENFSTDLL SymbolTableImpl* Read(istream &strm, //ChangedPD
                               const SymbolTableReadOptions& opts);

  bool OPENFSTDLL Write(ostream &strm) const; //ChangedPD
 ...
 private:
  ...
  OPENFSTDLL void MaybeRecomputeCheckSum() const; //ChangedPD
...
class SymbolTable {
 public:
  ...
  virtual OPENFSTDLL void AddTable(const SymbolTable& table); //ChangedPD
  ...
  virtual OPENFSTDLL bool WriteText(ostream &strm) const; //ChangedPD
